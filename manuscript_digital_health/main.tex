...
\codeavailability{The repository at \url{https://github.com/Hajimi-Sudo/care-state-survey-risk} contains the CARE-State model implementation, survey-weighted evaluation metrics, validation-only calibration adapter, aggregate derived results, figure-generation scripts, manuscript sources, and documentation. These materials support model execution on a harmonised input table and reproduction of the reported aggregate figures. It does not contain individual-level NHANES or NHIS records or the original server-side source-data harmonisation pipeline. The public-use source files should be obtained from the links provided in the Data Availability Statement.}
